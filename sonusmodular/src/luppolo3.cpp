...
struct Luppolo3 : Module
{
    ...
    Luppolo3()
    {
        ...
        configParam(Luppolo3::DIRECTION, 0.0, 1.0, 0.0, "Direction");
        configParam(Luppolo3::ERASE, 0.0, 1.0, 0.0, "Erase");
        configParam(Luppolo3::GAIN_M, 0.0, 2.0, 1.0, "Gain Master");
        configParam(Luppolo3::TRIGGER_M, 0.0, 1.0, 0.0, "Trigger Master");
        configParam(Luppolo3::OVERDUB_M, 0.0, 1.0, 0.0, "Overdub Master");
        configParam(Luppolo3::CLEAR_M, 0.0, 1.0, 0.0, "Clear Master");
        configParam(Luppolo3::GAIN_S1, 0.0, 2.0, 1.0, "Gain Slave 1");
        configParam(Luppolo3::TRIGGER_S1, 0.0, 1.0, 0.0, "Trigger Slave 1");
        configParam(Luppolo3::OVERDUB_S1, 0.0, 1.0, 0.0, "Overdub Slave 1");
        configParam(Luppolo3::CLEAR_S1, 0.0, 1.0, 0.0, "Clear Slave 1");
        configParam(Luppolo3::GAIN_S2, 0.0, 2.0, 1.0, "Gain Slave 2 ");
        configParam(Luppolo3::TRIGGER_S2, 0.0, 1.0, 0.0, "Trigger Slave 2");
        configParam(Luppolo3::OVERDUB_S2, 0.0, 1.0, 0.0, "Overdub Slave 2");
        configParam(Luppolo3::CLEAR_S2, 0.0, 1.0, 0.0, "Clear Slave 2");
    }
...
void Luppolo3::process(const ProcessArgs &args)
{
    ...
    if (((inputs[TRIGGER_DIR].getVoltage() != 0.0) && dir_last_value[0] == 0.0) || ((std::roundf(params[DIRECTION].getValue()) != 0.0) && (dir_last_value[1] == 0.0)))
    {
        forward = !forward;
    }

    dir_last_value[0] = inputs[TRIGGER_DIR].getVoltage();
    dir_last_value[1] = std::roundf(params[DIRECTION].getValue());

    // Master track
    if (((inputs[CV_TRIGGER_M].getVoltage() != trig_last_value[0][0]) && (trig_last_value[0][0] == 0.0)) || ((std::roundf(params[TRIGGER_M].getValue()) != trig_last_value[0][1]) && (trig_last_value[0][1] == 0.0)))
    {
        if (!is_recording[0])
        {
            for (int c = 0; c < 2; c++)
    ...
    trig_last_value[0][1] = std::roundf(params[TRIGGER_M].getValue());

    if (((inputs[CV_OVERDUB_M].getVoltage() != overdub_last_value[0][0]) && (overdub_last_value[0][0] == 0.0)) || ((std::roundf(params[OVERDUB_M].getValue()) != overdub_last_value[0][1]) && (overdub_last_value[0][1] == 0.0)))
    {
    ...
    overdub_last_value[0][1] = std::roundf(params[OVERDUB_M].getValue());

    if (((inputs[CV_CLEAR_M].getVoltage() != 0.0) && clear_last_value[0][0] == 0.0) || ((std::roundf(params[CLEAR_M].getValue()) != 0.0) && (clear_last_value[0][1] == 0.0)))
    {
        master_rec = false;
    ...
    clear_last_value[0][1] = std::roundf(params[CLEAR_M].getValue());
    ...
    if (master_rec)
    {
        for (int t = 1; t < 3; t++)
        {
            ...
            if (((inputs[CV_TRIGGER_M + t].getVoltage() != trig_last_value[t][0]) && (trig_last_value[t][0] == 0.0)) || ((std::roundf(params[TRIGGER_M + t].getValue()) != trig_last_value[t][1]) && (trig_last_value[t][1] == 0.0)))
            {
                if (!is_recording[t])
                {
                    for (int c = 0; c < 2; c++)
            ...
            trig_last_value[t][1] = std::roundf(params[TRIGGER_M + t].getValue());

            if (((inputs[CV_OVERDUB_M + t].getVoltage() != overdub_last_value[t][0]) && (overdub_last_value[t][0] == 0.0)) || ((std::roundf(params[OVERDUB_M + t].getValue()) != overdub_last_value[t][1]) && (overdub_last_value[t][1] == 0.0)))
            {
            ...
            overdub_last_value[t][1] = std::roundf(params[OVERDUB_M + t].getValue());


            if (((inputs[CV_CLEAR_M + t].getVoltage() != 0.0) && clear_last_value[t][0] == 0.0) || ((std::roundf(params[CLEAR_M + t].getValue()) != 0.0) && (clear_last_value[t][1] == 0.0)))
            {
            ...
            clear_last_value[t][1] = std::roundf(params[CLEAR_M + t].getValue());
    ...
    if (((inputs[CV_ERASE].getVoltage() != 0.0) && erase_last_value[0] == 0.0) || ((std::roundf(params[ERASE].getValue()) != 0.0) && (erase_last_value[1] == 0.0)))
    {
        master_rec = false;
        for (int t = 0; t < 3; t++)
        {
    ...
    erase_last_value[1] = std::roundf(params[ERASE].getValue());
}
