...
struct Addiction : Module
{
    ...
    Addiction()
    {
        ...
        configParam(OCTAVE, -3.0, 3.0, 0.0, "Base Frequency Octave");

        const char* intervals[4] = {"Fund", "Min 3rd", "Maj 3rd", "Perf 5th"};
        
        for (unsigned int row = 0; row < 4; row++) {
            for (unsigned int interval = 0; interval < 4; interval++) {
                int paramId = 1 + (row * 4) + interval;  // +1 to skip OCTAVE
                std::string label = string::f("H %d - %s", row + 1, intervals[interval]);
                configParam(paramId, 0.0, 1.0, def_amps[interval], label);
            }
        }
    }
...
void Addiction::process(const ProcessArgs &args)
{
    ...
    for (unsigned int i = 0; i < 16; i++)
    {
        ...
        float currentVoltage = outputs[OUTPUT].getVoltage();
        outputs[OUTPUT].setVoltage(currentVoltage + sin(ramps[i] * M_PI) * (params[i + 1].getValue() + (inputs[i + 1].getVoltage() * 0.2)));
        amp_sum += (params[i + 1].getValue() + (inputs[i + 1].getVoltage() * 0.2));
    }

    if (amp_sum > 0.0)
    {
        float finalVoltage = outputs[OUTPUT].getVoltage() / (amp_sum * 0.2);
        outputs[OUTPUT].setVoltage(finalVoltage);
    }
    else
    {
        outputs[OUTPUT].setVoltage(0.0);
