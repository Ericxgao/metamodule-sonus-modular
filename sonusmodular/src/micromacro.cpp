...
struct Micromacro : Module
{
    ...
    Micromacro()
    {
        ...
        configParam(Micromacro::BPM, 20.0, 220.0, 120.0, "BPM");
        configParam(Micromacro::STEPS, 1.0, 16.0, 16.0, "Steps");
        configParam(Micromacro::BASE_OCTAVE, -3.0, 3.0, 0, "Base Octave");
        configParam(Micromacro::BASE_NOTE, 0.0, 11.0, 0.0, "Base Note");
        configParam(Micromacro::TUNE, 432.0, 448.0, 440.0, "Tune");
        configParam(Micromacro::INTERVALS, 0.0, 2.0, 0.0, "Intervals");
        configParam(Micromacro::CLOCK_SOURCE, 0.0, 1.0, 0.0, "Clock Source");
        configParam(Micromacro::RESET, 0.0, 1.0, 0.0, "Reset");
        configParam(Micromacro::PLAY, 0.0, 1.0, 1.0, "Play");
        for (unsigned int s = 0; s < 16; s++)
        {
            configParam(Micromacro::PITCH_1_1 + (s * 2), 0.0, 17.0, 0.0, "Pitch 1-1 " + std::to_string(s + 1));
            configParam(Micromacro::GATE_1_1 + (s * 2), 0.0, 1.0, 0.0, "Gate 1-1 " + std::to_string(s + 1));
            configParam(Micromacro::PITCH_1_2 + (s * 2), 0.0, 17.0, 0.0, "Pitch 1-2 " + std::to_string(s + 1));
            configParam(Micromacro::GATE_1_2 + (s * 2), 0.0, 1.0, 0.0, "Gate 1-2 " + std::to_string(s + 1));
            configParam(Micromacro::PITCH_1_3 + (s * 2), 0.0, 17.0, 0.0, "Pitch 1-3 " + std::to_string(s + 1));
            configParam(Micromacro::GATE_1_3 + (s * 2), 0.0, 1.0, 0.0, "Gate 1-3 " + std::to_string(s + 1));
        }
    }
...
void Micromacro::process(const ProcessArgs &args)
{
    ...
    clock_source_ext = roundf(params[CLOCK_SOURCE].getValue()) > 0;
    ...
    type = (unsigned int)(roundf(params[INTERVALS].getValue()));
    paused = roundf(params[PLAY].getValue()) <= 0;
    play_last_value = params[PLAY].getValue();

    for (unsigned int s = 0; s < 16; s++)
    {
        s == current_step ? lights[STEP_LED_1 + s].value = 1.0 : lights[STEP_LED_1 + s].value = 0.0;
        steps[s][0].gate = roundf(params[GATE_1_1 + (s * 2)].getValue()) > 0;
        steps[s][1].gate = roundf(params[GATE_1_2 + (s * 2)].getValue()) > 0;
        steps[s][2].gate = roundf(params[GATE_1_3 + (s * 2)].getValue()) > 0;
    ...
    if ((inputs[CV_RESET].getVoltage() != 0.0 && reset_last_value[0] == 0.0) || (roundf(params[RESET].getValue()) > 0 && reset_last_value[1] == 0.0))
    {
        current_step = 0;
    }
