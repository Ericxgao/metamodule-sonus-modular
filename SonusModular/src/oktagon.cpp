...
struct Oktagon : Module
{
    ...
    Oktagon()
    {
        ...
        configParam(Oktagon::RANGE, 0.0, 1.0, 0.0, "Range");
        configParam(Oktagon::FREQUENCY, -2.0, 2.0, 0.0, "Frequency");
...
void Oktagon::process(const ProcessArgs &args)
{
    ...
    lights[FREQ_LIGHT].value = (outputs[WAVE0_OUT].getVoltage() > 0.0) ? 1.0 : 0.0;
}
