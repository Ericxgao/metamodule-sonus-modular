...
void Campione::process(const ProcessArgs &args)
{
    ...
    if (((inputs[CV_RECORD].getVoltage() != rec_last_value[0]) && (rec_last_value[0] == 0.0)) || ((std::roundf(params[RECORD].getValue()) != rec_last_value[1]) && (rec_last_value[1] == 0.0)))
    {
        if (!recording)
        {
            sample.clear();
    ...
    rec_last_value[1] = std::roundf(params[RECORD].getValue());

    if (((inputs[CV_DIRECTION].getVoltage() != dir_last_value[0]) && (dir_last_value[0] == 0.0)) || ((std::roundf(params[DIRECTION].getValue()) != dir_last_value[1]) && (dir_last_value[1] == 0.0)))
    {
        forward = !forward;
    }
    ...
    if (one_shot)
    {
        if (((inputs[CV_PLAY].getVoltage() != play_last_value[0]) && (play_last_value[0] == 0.0)) || ((std::roundf(params[PLAY].getValue()) != play_last_value[1]) && (play_last_value[1] == 0.0)))
        {
            playing = !playing;
    ...
    {
        if ((inputs[CV_PLAY].getVoltage() != 0.0) || (std::roundf(params[PLAY].getValue()) != 0.0))
        {
            playing = true;
        }
        else
    ...
    play_last_value[1] = std::roundf(params[PLAY].getValue());
    ...
    if (((inputs[CV_CLEAR].getVoltage() != clear_last_value[0]) && (clear_last_value[0] == 0.0)) || ((std::roundf(params[CLEAR].getValue()) != clear_last_value[1]) && (clear_last_value[1] == 0.0)))
    {
        sample.clear();
        pos = 0.0;
    }
    ...
    clear_last_value[1] = std::roundf(params[CLEAR].getValue());
