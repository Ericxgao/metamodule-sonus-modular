...
struct Pusher : Module
{
    ...
    Pusher()
    {
        ...
        configParam(Pusher::CV_1, -5.0, 5.0, 5.0, "CV 1");
        configParam(Pusher::CV_2, -5.0, 5.0, 5.0, "CV 2");
        configParam(Pusher::CV_3, -5.0, 5.0, 5.0, "CV 3");
        configParam(Pusher::CV_4, -5.0, 5.0, 5.0, "CV 4");
        configParam(Pusher::PUSH_1, 0.0, 1.0, 0.0, "Push 1");
        configParam(Pusher::PUSH_2, 0.0, 1.0, 0.0, "Push 2");
        configParam(Pusher::PUSH_3, 0.0, 1.0, 0.0, "Push 3");
        configParam(Pusher::PUSH_4, 0.0, 1.0, 0.0, "Push 4");
...
{
    outputs[OUT_1].setVoltage(params[CV_1].getValue() * roundf(params[PUSH_1].getValue()));
    outputs[OUT_2].setVoltage(params[CV_2].getValue() * roundf(params[PUSH_2].getValue()));
    outputs[OUT_3].setVoltage(params[CV_3].getValue() * roundf(params[PUSH_3].getValue()));
    outputs[OUT_4].setVoltage(params[CV_4].getValue() * roundf(params[PUSH_4].getValue()));
}
