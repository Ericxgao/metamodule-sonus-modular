...
struct Mrcheb : Module
{
    ...
    Mrcheb()
    {
        ...
        configParam(Mrcheb::S1K, 0.0, 1.0, 0.2, "S1");
        configParam(Mrcheb::S2K, 0.0, 1.0, 0.1, "S2");
        configParam(Mrcheb::S3K, 0.0, 1.0, 0.1, "S3");
        configParam(Mrcheb::S4K, 0.0, 1.0, 0.1, "S4");
        configParam(Mrcheb::S5K, 0.0, 1.0, 0.1, "S5");
        configParam(Mrcheb::S6K, 0.0, 1.0, 0.1, "S6");
        configParam(Mrcheb::S7K, 0.0, 1.0, 0.1, "S7");
        configParam(Mrcheb::S8K, 0.0, 1.0, 0.1, "S8");
        configParam(Mrcheb::S9K, 0.0, 1.0, 0.1, "S9");
        configParam(Mrcheb::RANGE, 0.0, 1.0, 1.0, "Range");
...
void Mrcheb::process(const ProcessArgs &args)
{
    ...
    if (roundf(params[RANGE].getValue()) == 0.0)
    {
        bipolar = false;
    }
    ...
    for (int k = 0; k < 9; k++)
    {
        output += outputs[k].getVoltage() * params[k].getValue();
